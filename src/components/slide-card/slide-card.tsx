...
const SlideCard = ({item}: {item: ISlideCard}) => {
  return (
    <div id={item.id} className={`${item.itemWidth ? "w-[600px]" : "w-[60px]"} item-card h-[400px] cursor-pointer bg-[url('${item?.link}')] bg-no-repeat bg-cover bg-center flex items-end justify-start p-2 rounded-[30px]`}>
