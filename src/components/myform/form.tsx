...
const ContactForm = () => {
  ...
  const [checkMessageData, setCheckMessageData] = useState<IMess>({
    mess: "",
    check: false,
  });
  const sendMessage = async (data: IData) => {
    ...
    try {
      ...
      setCheckMessageData({ mess: "Success!", check: true });
      reset();
    } catch (error) {
      setLoadingBtn(false);
      setCheckMessage(true);
      setCheckMessageData({ mess: "Error!", check: false });
    }
  ...
  return (
    <>
      {checkMessage && (
        <Alert
          content={checkMessageData?.mess}
          test={checkMessageData?.check}
        />
      )}
      <div className="container mx-auto laptop:mb-[70px]">
        <form
          className="px-[30px] pb-[45px] flex flex-wrap gap-x-5 justify-between bg-custom-white-10 -translate-y-28 rounded-b-[25px]"
        ...
        >
          ...
          <div className="w-full">
            ...
            >
              {loadingBtn ? "Loading..." : t("btn-contact")}
            </button>
          </div>
        </form>
      </div>
